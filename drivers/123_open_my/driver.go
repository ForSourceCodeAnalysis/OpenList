...
type Open123my struct {
	model.Storage
	Addition
	UID uint64
}

func (d *Open123my) Config() driver.Config {
	return config
}

func (d *Open123my) GetAddition() driver.Additional {
	return &d.Addition
}

func (d *Open123my) GetUploadInfo() *model.UploadInfo {
...
func (d *Open123my) Init(ctx context.Context) error {
...
func (d *Open123my) Drop(ctx context.Context) error {
	op.MustSaveDriverStorage(d)
	return nil
}

func (d *Open123my) List(ctx context.Context, dir model.Obj, args model.ListArgs) ([]model.Obj, error) {
	fileLastId := int64(0)
...
func (d *Open123my) Link(ctx context.Context, file model.Obj, args model.LinkArgs) (*model.Link, error) {
...
func (d *Open123my) MakeDir(ctx context.Context, parentDir model.Obj, dirName string) error {
	parentFileId, _ := strconv.ParseInt(parentDir.GetID(), 10, 64)

	return d.mkdir(parentFileId, dirName)
}

func (d *Open123my) Move(ctx context.Context, srcObj, dstDir model.Obj) error {
	toParentFileID, _ := strconv.ParseInt(dstDir.GetID(), 10, 64)

	return d.move(srcObj.(File).FileId, toParentFileID)
}

func (d *Open123my) Rename(ctx context.Context, srcObj model.Obj, newName string) error {
	fileId, _ := strconv.ParseInt(srcObj.GetID(), 10, 64)

	return d.rename(fileId, newName)
}

func (d *Open123my) BatchRename(ctx context.Context, obj model.Obj, renameObjs []model.RenameObj) error {
	rl := []string{}
...
func (d *Open123my) Copy(ctx context.Context, srcObj, dstDir model.Obj) error {
	// 尝试使用上传+MD5秒传功能实现复制
	// 1. 创建文件
...
// Remove 删除文件
func (d *Open123my) Remove(ctx context.Context, obj model.Obj) error {
...
// BatchRemove 批量删除
func (d *Open123my) BatchRemove(ctx context.Context, srcObj model.Obj, objs []model.IDName) error {
...
// Put 单次上传
func (d *Open123my) Put(ctx context.Context, dstDir model.Obj, file model.FileStreamer, up driver.UpdateProgress) error {
	parentFileID, err := strconv.ParseInt(dstDir.GetID(), 10, 64)
...
func (d *Open123my) GetDetails(ctx context.Context) (*model.StorageDetails, error) {
...
// Preup 预上传
func (d *Open123my) Preup(c context.Context, srcobj model.Obj, req *reqres.PreupReq) (*model.PreupInfo, error) {
...
// UploadSlice 上传分片
func (d *Open123my) SliceUpload(c context.Context, req *tables.SliceUpload, sliceno uint, fd multipart.File) error {
...
func (d *Open123my) UploadSliceComplete(c context.Context, su *tables.SliceUpload) error {

	return d.sliceUpComplete(su.PreupID)
}

var _ driver.Driver = (*Open123my)(nil)
